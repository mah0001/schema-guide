...
We describe in this chapter the features that an on-line data catalog should have to better satisfy the needs and expectations of various categories of users. The objective is develop catalogs that combine lexical search, semantic search (NLP), filtering, advanced search option (targeted), and a recommender system. Also, that provide users with detaled metadata, and convenience in formats and modes of access. 

## Features of a modern data catalog  

Metadata plays a crucial role in making data both usable and discoverable. To make data usable, comprehensive and detailed metadata should be provided to data users in a format that is convenient for them, such as HTML, JSON, PDF, or XML. To make data discoverable, metadata should be indexed and searchable in generic and specialized search engines, and useful filtering options (facets) should be provided in data catalogs. Data must also be made visible by implementing search engine optimization.

To achieve the objectives of usability, discoverability and visibility, structured metadata should be published in online data catalogs. Our Guide serves as a complement to our NADA open-source cataloging application, but other online data cataloging applications such as CKAN serve similar purposes, although not all of them will enforce compliance with the metadata standards we recommend. The core features of a modern data catalog are not specific to the NADA application.

The process of publishing data and metadata in a NADA catalog involves four steps:

1. **Documenting the dataset** by generating structured metadata that complies with the supported standards and schemas. This can be done using a specialized metadata editor, a programming language such as R or Python, or procedures of metadata augmentation to make the metadata as rich as possible.

2. **Uploading the metadata** into NADA, either through the NADA administrator interface or programmatically through the NADA API. The metadata will be automatically indexed by the default MySQL engine or, for a better-performing catalog, using the optional Solr application. NADA will also automatically generate a DECAT metadata file for the dataset, which is only intended for search engine optimization purposes and contains a simplified version of the uploaded metadata.

3. **Uploading the resource files or providing links to these resources.** These resources include all files and links related to the dataset that the catalog administrator wants to make available to users. A brief description of each resource should be provided, compliant with the external resources schema.

4. **Selecting an access policy** that will be applied to the dataset, when relevant. This will determine how users can access the data, such as direct access, access from another repository, licensed access, no access, or other. This policy typically applies to microdata and geographic datasets, but not to other types of data.
...
When it comes to on-line data catalogs, users expect a seamless and intuitive experience that allows them to quickly access the most relevant datasets. This requires a combination of user-friendly **search** tools and **filters**, also known as facets. To meet these expectations, we can draw inspiration from the design of successful search engines like Google or Bing, as well as e-commerce platforms like Amazon. These platforms are characterized by their simplicity, predictability, relevance, speed, and reliability, all of which should be incorporated into the design of data catalogs. By providing a user-friendly experience that mimics the ease of use and convenience of popular search engines and e-commerce sites, data catalogs can help users find and acquire the data they need quickly and easily.

#### Filters / facets

Facets, also known as filters, are a valuable tool in narrowing down datasets based on categorical metadata elements. For example, if a data catalog contains datasets from multiple countries, a "country" facet will allow users to quickly and easily find the datasets relevant to their needs. For filters to be effective, they must be based on metadata elements that have a limited number of categories and a predictable set of options. Controlled vocabularies can be used in data documentation to enable such filters. Additionally, as some metadata elements are specific to certain data types, contextual facets should be implemented in the catalog's user interface to provide relevant filters based on the type of data being searched.
...
The search engine in a data catalog plays a crucial role in identifying relevant datasets based on user queries. There are two types of search engines - lexical and semantic. The former matches literal terms in the query to the search engine's index, while the latter aims to identify datasets that have semantically similar metadata to the query (see sections 1.2.3 and 1.8 of the Guide).

While an ideal data catalog would offer both types of search engines, implementing semantic searchability can be complex. In the absence of semantic search, keyword-based searches can still be optimized using tools like Solr or ElasticSearch. Out-of-the-box solutions, such as those provided by SQL databases, rarely deliver satisfactory results. Structured metadata can help optimize search engines by allowing for **boosting** of specific metadata elements. For instance, a query term found in the *title* of a dataset would carry more weight than if it were found in the *notes* element. Similarly, a country name found in the *nation* or *reference country* elements should be given more weight than if it were found in a variable description. Advanced indexing tools like Solr and ElasticSearch provide multiple functionalities to fine-tune search engines and enhance result relevancy.

Although most search engines rely on a single search box, structured metadata enables developers to build advanced search interfaces that allow users to search for keywords in specific metadata elements, such as the authors element of a document.

A search engine should be able to process short or long queries, even accepting a document as a query in the case of semantic searchability. For more detailed information, see D. Turnbull and J. Berryman's (2016) in-depth description of tools and methods.
