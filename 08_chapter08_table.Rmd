...
- **`id_numbers`** *[Optional ; Repeatable]* <br>
...
- **`data_sources`** *[Optional, Repeatable]* <br>     @@@@ redo screenshot; has more elements than shown
The data sources are often cited in the footnote section of a table. The `name`, `source_id`, and `link` elements are optional, but at least one of them must be provided.
    ...
    - **`source_id`** *[Optional, Not repeatable, String]*  @@@@@@@@@@ added - need new screenshot
    A unique identifier for the source, such as a Digital Object Identifier (DOI).
    - **`link`** *[Optional, Not repeatable, String]* <br>
...
- **`themes`** *[Optional ; Repeatable]* <br>  @@@@@@@@@@@ redo screenshot - incomplete
  <br>
...
- **`disciplines`** *[Optional ; Repeatable]* <br>   @@@@@ some missing elements in ReDoc; add then redo screenshot
  <br>
...
- **`sdc`** *[Optional, Not repeatable, String]* <br>  @@@@@@@@@@@@ add to the schema; do screenshot
...
We provide here examples of documentation of actual tables, and their publishing in a NADA catalog. We use the R package NADAR and the Python library PyNada to publish metadata in the catalog.
...
# Publish the table in a NADA catalog with a link to US Bucen website

table_add(idno = my_table_hh4$table_description$title_statement$idno, 
...
external_resources_add(
  ...
  idno = my_table_hh4$table_description$title_statement$idno,
  dctype = "web",
...
The result in NADA will look like (not full metadata displayed):
@@@@ add screenshots
...
tbl_wdi_indicators = list("SP.POP.TOTL", "SP.POP.GROW", "AG.SRF.TOTL.K2", "EN.POP.DNST",
...
# We only include here a subset of geographies for which the table is available.
...
setwd("C:/Users/wb147665/OneDrive - WBG/_OD/SCHEMA_GUIDE/MISC")

# --------------------------------------------------------------------------
# Catalog administrator credentials
# --------------------------------------------------------------------------

# Set catalog URL - The on-line catalog where the tables will be published
catalog_url <- "http://my_NADA_catalog/index.php/api"
set_api_url(catalog_url)
# Enter an API key that provides administrator access to the catalog
# We read it from an external file to keep it confidential. 
...
df <- read_xlsx("WV.2_Global_goals_ending_poverty_and_improving_lives.xlsx", 
                range = "A5:A230")
ctry_list <- list()
for(i in 1:nrow(df)) {
...
# We extract the definitions found in the table; note that we could have 
# instead copy/pasted the definitions. For example, the command line:
...
# We can now generate the table metadata
...
# We add external resources and publish them 
# (The MS-Excel and PDF versions of the table)
...
## References
   ...
   -https://www.emathzone.com/tutorials/basic-statistics/construction-of-statistical-table.html#:~:text=Only%20the%20first%20letter%20of,must%20be%20written%20in%20lowercase.&text=The%20horizontal%20headings%20and%20sub,written%20is%20called%20the%20stub.
   -https://www.preservearticles.com/articles/parts-of-a-statistical-table/3767
   -https://www.coursehero.com/file/p7rj052/Major-Functional-Parts-of-Statistical-Table-a-Table-Number-Each-table-must-be/
   -https://labwrite.ncsu.edu/res/gh/gh-tables.html
   -https://www.csprousers.org/help/CSPro/parts_of_a_table.html (added this)
   -https://www.vedantu.com/commerce/tabulation(added this)
   -https://apastyle.apa.org/style-grammar-guidelines/tables-figures/tables (example of specific publications styles for a table)
