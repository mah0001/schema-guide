...
Search engines like Google and Bing are designed to search the whole world wide web, not specifically to find data. They perform remarkably well for simple and clearly-defined data queries (e.g., "GDP per capita India 2020"), but will not always provide relevant results to more complex or specialized queries that researchers and data analysts may submit. When looking for data, researchers will often turn to specialized on-line data catalogs, like the open data platforms and catalogs maintained by national or international organizations and academic data centers and libraries. Most often, the search engine embedded in such catalogs are out-of-the-box solutions and/or are not optimized, and perform poorly. Google's *Dataset Search* is an attempt to apply the performance of Google's search engine to a specialized data catalog. But it's performance is hampered by the fact that Google rely on metadata - rarely complete - generated by external parties. The few examples below (generated in January 2022; results may differ over time) illustrate in an anecdotal manner some of the problems that researchers may face when searching for data.
