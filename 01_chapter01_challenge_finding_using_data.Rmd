...
General search engines like Google and Bing have remarkable capabilities in locating and ranking relevant resources available on-line and in some cases returning instant answers to users' queries (a trend that will accelerate with the emergence of solutions like chatGPT). But these search engines are designed to answer relatively simple queries, and are not optimized to find or recommend data that can best help researchers answer complex research questions. Some of these data may be available from data libraries or repositories that implement advanced curation and search engine optimization. But others may be found in more hidden locations such as article supplements, available as "data services" and not as datasets, non-optimized data catalogs, and may have limited metadata that make them largely invisible or "unattractive" to web crawlers. Since data are scattered across many sources and locations, users without prior knowledge of what data they need and where to find them will often be poorly served. Google's *Dataset Search* is one attempt to implement a better search engine to a catalog of datasets. But its performance is hampered by the paucity of available metadata. Other data aggregators face the same issue.

How do researchers find data? Tribal knowledge, cited datasets (not always easy), prior knowledge of where they may be located (ICPSR), or the web. Very specialized users probably have good knowledge of relevant repositories. But the user community is much broader. So there is a need to make data discoverable and accessible to all to democratize their access and use. And the research community should be considered in a broad sense.   

Finding a specific dataset may be easy. For example, finding "LSMS Albania 2002". But finding datasets based on their content is much more difficult. For example, "pre-school attendance and child malnutrition". Would that query find the Albania 2012 dataset, which contains variables on age, school attendance, level attended, height, and weight? Note that many papers cite data improperly. It is sometimes difficult to identify and locate -then acquire- a dataset used by a researcher. "Planning to incorporate persistent identifiers into the research workflow is a straightforward way to design for findability." Finding a dataset requires first identifying it clearly, which is not always easy.

When we search for data, we make the computer search metadata. Metadata is defined as "... structured information that describes, explains, locates, or otherwise makes it easier to retrieve, use or manage that resource" (Data thesaurus, NIH, https://nnlm/gov/data/thesaurus) Metadata must be findable by machines and usable by humans.
