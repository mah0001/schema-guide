...
The leading search platforms like Google and Bing have outstanding capabilities. They are not only able to locate resourecs of interest on the web, but also increasingly to provide direct answers to their users and to becaome an assistant. The emergence and use of tools like chatGPT will only increase this trend. These systems are however not tailored to meet the needs of researchers looking for data. Simple requests will be answered directly. For example, someone who wants to know the population of India in 2020 will eb provided with a direct answer, extracted by the search engine from a source deemed (by them, not by the user) as authoritative. But these systems will not always return the most useful results to researchers whose data needs cannot be expressed in a simple query. A research question may be answered by multiple types of data, some available in data libraries or repositories, other available as somewhat hidden links in websites (e.g., as an article supplement).

Many users will not have a query for a specific indicator or dataset, but need recommendations on useful data to answer a research question. How do researchers look for data when they do not know exactly what data they need to find:
