...
Ideally, the researcher should not only be provided with links to relevant datasets but also with easy access to the metadata they need to assess the relevance of the data for their specific purpose. Acquiring a dataset may be time consuming, and sometimes involve other costs. Users should only invest time and resources in the acquisition of data for which they have assurance of quality and relevancy.

Assessing the fitness for purpose of a dataset will require different metadata for different data types and uses. Some metadata elements are "obvious": data type, geographic coverage, temporal coverage, scope and universe, and access policy. But more detailed information may be needed. For example, a survey dataset (microdata) may only be relevant to a user if a specific modality of a specific variable had a sufficient number of respondents (e.g., a researcher interested in mental issues by age may find a study that contains a variable "type of disability" with a modality "mental disability", but if the sample size is such that the number of observations corresponding to this modality is very small, the dataset would not allow any valid statistical inference). Comparability across sources is also essential to many uses and users, so the metadata should provide detailed description of sampling, universe, variables, concepts, and methods as relevant to the data type. A data user may also be interested in information on the periodicity of updates of the data (for time series or panel surveys, for example), and on past uses of the dataset by the research community.

Even when access to the data is restricted, detailed metadata should thus be provided openly an in a user-friendly interface to the users. 


## Accessing data

Responsible users will want to know that their specific use of the data complies with the legal, ethical, and technical requirements imposed by the owner or distibutor of the dataset. All datasets should be provided with detailed information on the attached terms of use. A standard license may be used. The conditions 

For datasets that are not provided with an option for direct download, an easy-to-use procedure should be implemented by the data distributor. What information must be provided by the user, how long it takes to obtain a response to a request for access to data, what appeal options may be available in case of refusal, how personal information included in the access request forms will be processed and used, what registration process is required, etc.. must be very transparent and user-friendly.

The modes of access to the data should also accommodate different users preferences. The modalities may include: 
- Bulk download, in which case the data may be provided in multiple "recent" formats, at least one of them non-proprietary. 
- API, in which case detailed documentation of the API must be provided (legal and technical). 
- UI interface
- Visualizations
- Others
