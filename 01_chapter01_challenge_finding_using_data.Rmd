...
The leading search platforms like Google and Bing have outstanding capabilities. They are not only able to locate resources of interest on the web, but also increasingly to return instant answers to users's queries, instead of ranked links. For example, someone who wants to know the population of India in 2020 will be provided with a direct answer extracted by the search engine from a source deemed (by them, not by the user) to be authoritative. The emergence and use of tools like chatGPT will only increase this trend. But these systems are not tailored to meet the needs of researchers who do not look for an answer, but for data that can help them conduct analysis to answer a possibly complex research question. A researcher's question may require exploiting multiple types of data. Some of these data will be available in data libraries or repositories, other may be available in more hidden locations (e.g., as an article supplement). 

Because the data are scattered across many sources and locations, a user wo does not have prior knowledge of what data s/he needs and where they can be found will be poorly served. The lead search engines, although not dedicated to locating data, serve a crucial role to bring users if not to the data themselves, at least to the relevant catalogs and repositories. It is the responsibility of the administrators of these catalogs to ensure that their assets are visible in generic search engines, and that their specific applications provide additionsl more optimized search and discovery features. 

 Ideally, all data would be shared in compliance with the FAIR principles and be Findable, Accessible, Interoperable, and Reusable.
