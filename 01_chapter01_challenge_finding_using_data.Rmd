...
Researchers often face challenges in finding, assessing, accessing, and using relevant data. This process can be time-consuming and complex, requiring significant effort to identify, locate, acquire, understand, and manage data. In some cases, researchers may rely on personal networks or convenient data sources rather than seeking out the most relevant data. The emergence and use of tools like chatGPT will only increase this trend. However, improving the accessibility and usability of data can encourage the reuse and repurposing of existing data, leading to greater return on investment in data collection and creation. In this chapter, we will explore several examples that highlight the obstacles researchers encounter when searching for and utilizing data. 

## Finding data

Leading search platforms like Google and Bing are renowned for their remarkable capabilities in locating resources on the web and returning instant answers to users' queries. For example, someone who wants to know the population of India in 2020 will be provided with a direct answer extracted by the search engine from a source deemed (by the search engine, not by the user) to be authoritative. These search engines are designed to answer relatively simple questions and to provid access to specific resources available online, rather than to find relevant data that can help researchers conduct analyses to answer complex research questions. As researchers often require multiple types of data, finding and accessing relevant data can be a challenging task. Some data may be available in data libraries or repositories, while others may be found in more hidden locations such as article supplements. Since data are scattered across many sources and locations, users without prior knowledge of what data they need and where to find them will be poorly served. Google's *Dataset Search* is an attempt to implement a better search engine to a catalog of datasets. But its performance is hampered by the paucity of available metadata. 

While search engines like Google and Bing play a crucial role in bringing users to relevant catalogs and repositories, specialized online data catalogs and platforms maintained by national or international organizations, academic data centers, data archives, or data libraries are better suited for researchers seeking relevant data. However, the search algorithms embedded in data catalogs can also provide unsatisfactory search results due to weaknesses in metadata associated with data resources and the lack of optimization of search indexes and algorithms.

Ideally, all data should be shared in compliance with the FAIR principles to ensure that they are Findable, Accessible, Interoperable, and Reusable. Google's Dataset Search is a noteworthy attempt to improve the search engine's performance in finding relevant datasets, but its efficacy is hindered by the paucity of available metadata.

In conclusion, while search engines like Google and Bing are useful for finding specific resources online, specialized online data catalogs and platforms are more effective in meeting the needs of researchers seeking relevant data. Administrators of these catalogs have a responsibility to ensure that their assets are visible in generic search engines and that their specific applications provide additional optimized search and discovery features. 
