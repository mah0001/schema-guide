...
**Example 1: Searching for data on education and inequality in Kenya using Google**

Suppose an analyst wants to study the correlation between education and inequality in Kenya. Such an analysis requires microdata from household surveys. A Google search for "data on inequality and education in Kenya" returns useful links, mostly to publications and blogs. Among the top 40 results, one provides a link to the website of the Kenya Statistics Office, but none provides a link to a specific dataset. A more specific query for "survey microdata on inequality and education in Kenya" returns one relevant result, a link to the *Kenya Integrated Household Budget Survey 2015-2016* in DataFirst, the data library of the University of Cape Town. However, this result is due to luck. DataFirst published the metadata in a collection titled "Centre of Excellence for Inequality Research," where the keyword inequality was found. The results returned by Google do not include some other relevant survey datasets that contain variables on education and on household income or expenditures (from which inequality measures can be derived), but whose metadata do not mention the word "inequality."

<br>
<center>
![image](https://user-images.githubusercontent.com/35276300/229384813-ad130de1-656a-4a81-a280-6c31342673c3.png){width=90%}
</center>
<br>
...
The success of a search query depends largely on the user's ability to formulate a clear query, but what may seem clear to a human may not be seen that way by search algorithms. Many search algorithms rely on keyword search, which means they look for an exact match between keywords in a query and keywords in the index (or a stemmed or lemmatized version of the keywords). However, this approach has limitations as synonyms or related terms may not be recognized. A search for data on "child malnutrition" illustrates this. Child malnutrition is typically measured by the percentage of children stunted, wasted, and overweight. Queries for "child malnutrition", "children stunted", "children wasted", and "children overweight" in the U.S. Government’s open data platform (data.gov) return different results, with little overlap between the results of the four queries. 
...
Both Google and chatGPT mention the WorldBank as the source. chatGPT provide results that differ from the information available in the World Bank website. And both fail to provide satisfactory metadata on the indicator; the label of the indicator itself is imprecise. Indeed, four different time series in the World Bank database may correspond to GDP per capita in US$.
