...
General search engines such as Google and Bing have remarkable capabilities in locating and ranking relevant resources available online, and in some cases, returning instant answers to users' queries. However, these search engines are designed to answer relatively simple queries and may not be able to find or recommend the most relevant data to help researchers answer complex research questions. Additionally, relevant data may be "hidden" in locations such as article supplements, attachments to publications, or available as data services and not as datasets. Other datasets may be available from open data repositories, but published with limited metadata, making them largely invisible to web crawlers. Due to data being scattered across many sources and locations and published with metadata that is not optimized for search engines, users without prior knowledge of what data they need and where to find it may be poorly served. Google's Dataset Search is one attempt to implement a better search engine for a catalog of datasets, but its performance is hampered by the paucity of available metadata.

While search engines like Google and Bing play a crucial role in bringing users to relevant catalogs and repositories, specialized online data catalogs and platforms maintained by national or international organizations, academic data centers, data archives, or data libraries are better suited for researchers seeking relevant data. These organizations provide a critical service of data curation. However, the search algorithms embedded in data catalogs can also provide unsatisfactory search results due to weaknesses in metadata associated with data resources and the lack of optimization of search indexes and algorithms. Administrators of these catalogs have a responsibility to ensure that their assets are visible in general search engines and that the search engines and other discovery tools implemented in their own catalogs provide an optimized user experience.

The examples provided below illustrate some of the problems that researchers encounter when searching for data.[2] 


**Example 1: Searching for data on education and poverty in Kenya using Google**

Suppose an analyst wants to study the correlation between education and poverty levels in Kenya. This requires microdata from household surveys. If the analyst queries "data on poverty and education in Kenya" on Google, the search results will provide useful links to publications with processed information. However, these results may not be as relevant for a researcher interested in conducting new analysis. A more specific query such as "survey microdata on poverty and education in Kenya" will return more relevant results, but may still miss some relevant datasets. Indeed, some datasets may contain variables on education levels and income without mentioning the keyword "poverty". Moreover, the metadata associated with many survey datasets may not include a detailed data dictionary, which significantly reduces the possibility for even the most advanced search algorithms to identify relevant resources.
...
What would be returned in an ideal world: publications with proper citations, documented derived variables in datasets, access to microdata, information on comparability, reproducible scripts, ...


**Example 2: Searching for *child malnutrition* in the [US Open Data platform (data.gov)](https://catalog.data.gov)**

The success of a search query depends to a large extent to the user's ability to formulate a clear query. But what may appear to be a clear query for a human may be seen as such by search algorithms. Many search algorithms are based on keyword search, i.e. they look for an exact match between keywords in a query and keywords in the index (or a stemmed or lemmatized version of the keywords). But this has limitations as synonyms or related terms may not be found. Child malnutrition is typically measured by the indicators of percentage of *children stunted*, *children wasted*, and *children overweight*. In a survey micro-dataset, one would need to find variables *age in months*, *height*, and *weight* for the dataset to be relevant. A query for *child malnutrition* in the U.S. Government’s open data platform [data.gov](https://catalog.data.gov) returns the following results: 
...
It is not clear whether the search algorithm has the capacity to identify relevant microdata based on variable names (it is unlikely that it does). But even an apparently less challenging task appears to be too complex for this (and for almost any other) data catalog. For more complex queries (for example, "impact of pre-school attendance on child malnutrition"), keywords-based search may not be the optimal solution. Semantic search and recommender systems would be more able to return useful results. 

What would help: semantic search, or augmented metadata keywords/topics.
...
chatGPT returns the following information.
![image](https://user-images.githubusercontent.com/35276300/229367133-ae5b1319-4682-46ef-8073-856f9e63149d.png){width=90%}

Both Google and chatGPT mention the WorldBank as source. Both provide results that differe from the information in the WB's website, and both fail to provide the basic necessary metadata on the indicator. GDP per capita in US$ corresponds to four different indicators.
![image](https://user-images.githubusercontent.com/35276300/229374548-6036a86f-d8c4-49bd-a916-73d7ad35ee79.png){width=90%}
![image](https://user-images.githubusercontent.com/35276300/229374735-9b7909a6-c1c9-4bac-9f3e-09c39f09bdf2.png){width=90%}
...
The main obstacles to data visibility, discoverability, and usability are that: 
   ...
   - **metadata is incomplete** and does not provide the full information that a user may need.

Official sources compete with others. Important for official statistics to be prioritized. Necessary (not sufficient condition) is that they optimize their documentation and dissemination.
