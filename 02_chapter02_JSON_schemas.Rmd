...
JSON is an open, non-propriertary format. JSON files are language-independent text files. Metadata in JSON are stored as *key/value* pairs, where the keys correspond to the names of the metadata elements in the standard. Values can be string, numeric, boolean, arrays, null, or JSON objects (for a more detailed description of the JSON format, see [www.w3schools.com](https://www.w3schools.com/js/js_json_intro.asp)). The example below shows how the JSON format is used to document the list of authors of a [document](http://hdl.handle.net/10986/34511), using elements from the Dublin Core metadata standard.
...
JSON files are easier to parse than XML, easier to generate programmatically (we will show in this Guide how JSON metadata schemas correspond to lists in R and to dictionaries in Python), and easier to read by humans. This makes them our preferred choice for describing and using metadata standards and schemas. 
...
The documentation of each standard or schema shows three or four common elements that are not actually elements of the standard or schema, but that contain information that will be used when the NADA cataloguing application is used to publish the metadata. If NADA is not used, these elements can be ignored.
...
This set of common elements is followed by one or multiple sections that contain the elements specific to each standard/schema. For example, the DDI Codebook metadata standard, used to document microdata, contains the following main sections:
...
The schema-specific sections are followed by a few other metadata elements that are common to most schemas. These are elements used to capture additional information useful for cataloguing and discoverability purposes. They include **tags** (which allow catalog administrators to attach tags to datasets independently of their type, which can be used as filters in the catalog), and **external resources**. 

Some schemas provide the possibility for data curators to add their own metadata elements, to accommodate **additional** information for which no metadata element is provided in the schema. The use of additional elements should be the exception, as metadata standards and schemas are expected to provide all necessary elements. 
...
The schemas may also provide default values and controlled vocabularies for some elements. 
