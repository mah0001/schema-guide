...
Metadata compliant with any of the standards and schemas described in this Guide can be generated in two different ways: programatically using a programming language like R or Python, or using a specialized metadata editor application.
...
We show below how the schemas described in their [on-line documentation](https://ihsn.github.io/nada-api-redoc/catalog-admin/#) translate into R scripts. Generating metadata using R will consist of producing a *list* object (itself containing lists).

The metadata schemas are [documented on-line](https://ihsn.github.io/nada-api-redoc/catalog-admin/#). In this documentation, curly brackets indicate that a list must be created to store the metadata elements. Square brackets indicate that a block of elements (the list) is repeatable, i.e. in R a list of list(s) will be expected. For example (extracted from the [DOCUMENT]((https://ihsn.github.io/nada-api-redoc/catalog-admin/#operation/createDocument)) metadata schema): 
...
We show below how the schemas described in their [on-line documentation](https://ihsn.github.io/nada-api-redoc/catalog-admin/#), translate into Python scripts. Generating metadata using Python will consist of producing a *dictionary* object (itself containing lists and dictionaries). Within this dictionary, metadata elements may be store as dictionaries or, for repeatable elements, as lists of dictionaries. 

The metadata schemas are [documented on-line](https://ihsn.github.io/nada-api-redoc/catalog-admin/#). In this documentation, curly brackets indicate that a dictionary must be created to store the metadata elements. Square brackets indicate that a block of elements (the dictionary) is repeatable. The same example we provided for R above: 
