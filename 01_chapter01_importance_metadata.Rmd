...
Data catalogs must be optimized to improve the visibility and ranking of their content in search engines, including specialized search engines like Google's [Dataset Search](https://datasetsearch.research.google.com/). The ranking of web pages by Google and other lead search engines is determined by a complex, proprietary, and non-disclosed algorithm. The only option for a web developer to ensure that a web page would appear on top of the Google list of results is to pay for it, publishing the page as a commercial ad. Otherwise, the ranking of a web page in the search engine results pages will be determined by many known and unknown elements. Lead search engines provide guidance and recommendations on search engine optimization (SEO) to web developers. See for example [Google Search Central](https://developers.google.com/search) where Google publish *Specific things you can do to improve the SEO of your website*. "Google's automated ranking systems are designed to present helpful, reliable information that's primarily created to benefit people, not to gain search engine rankings, in the top Search results." ([Google Search Central](https://developers.google.com/search/docs/fundamentals/creating-helpful-content))

Improving the ranking of catalog pages is a shared responsibiloity of data curators and catalog administrators.

**Data curators** must pay particular attention to providing rich, useful content in the catalog web pages (the HTML pages that describe each catalog entry). Remember that what search engines will use to identify your datasets as relevant is the metadata that will be indexed in their database. Datasets that are well documented, i.e. those published with rich and structured metadata, will rank higher. Much attention should be paid to include clear titles, descriptions (abstracts), list of keywords and topics covered, and identification of the geographic coverage of the data. In Google Search Central's terms: *Create helpful, reliable, people-first content.* and *Use words that people would use to look for your content, and place those words in prominent locations on the page, such as the title and main heading of a page, and other descriptive locations such as alt text and link text.* 

**Developers and administrators of cataloguing applications** must pay attention to: 

- Ensure that the catalog deliver a good page experience to users (see [Understanding page experience in Google Search results](https://developers.google.com/search/docs/appearance/page-experience)). This involves:
   - Catalog pages that load fast
   - Catalog pages that are mobile-friendly. A data catalog should be built with a responsive design.
...
- Embed *structured data* in the HTML catalog pages. Structured data is information included in web pages that help Google understand the content of a web page. It applies to certain types of content, including datasets. This influences not only the ranking, but also the way information on catalog web pages will be displayed in Google. The next section is dedicated to this.

A helpful and detailed [self-assessment list](https://developers.google.com/search/docs/fundamentals/creating-helpful-content) of items that data curators, catalog developers, and catalog administrators should pay attention to is provided by Google. Various tools are also available to catalog developers and administrators to assess the technical performance of their web pages (speed of loading, responsiveness, and others; see for example the [Core Web Vitals](https://web.dev/vitals-tools/) and [Google Search Console](https://developers.google.com/search/docs/monitor-debug/search-console-start)). 

The ranking of a web page in search engine results also depends much on the popularity and credibility ("authoritativeness") of the page. Google and other search engines will assess this by the number of other websites that provide links to a data catalog, and by the number of hits the catalog pages receive. As suggested by Google, *Tell people about your site. Be active in communities where you can tell like-minded people about your services and products that you mention on your site.*

### Structured data for rich results

What do we mean by "structured data" in this context? For what type of resources? https://developers.google.com/search/docs/appearance/structured-data/search-gallery
   microdata, indicators, geographic, tables, documents, images, videos. Also FAQs if catalog includes that. See list in Google page.
   
How does Google use it? 

How is it generated and where this is stored? How a catalog generates pages that users view?  Information in database --> extract ; adds visualizations if widgets ; generates html using CSS. WHen schema.org used, part of the extracted information (selected metadata elements) is stored as JSON-LD and saved in the HTML. For each schema, need to select elements, and map them to schema.org.   

"Google Search works hard to understand the content of a page. You can help us by providing explicit clues about the meaning of a page to Google by including structured data on the page. Structured data is a standardized format for providing information about a page and classifying the page content; for example, on a recipe page, what are the ingredients, the cooking time and temperature, the calories, and so on."  https://developers.google.com/search/docs/appearance/structured-data/intro-structured-data

Structured data is different for different types of resources. Google have multiple types of resources for which structured data can be provided. This includes receipes, etc. [link] The ones we are interested in are datasets, document, image. The structure of these data must be compliant with a schema from schema.org or with the DCAT metadata schema).

[Google guidelines - url]

According to their [developers' guide](https://developers.google.com/search/docs/data-types/dataset), Google's approach to dataset discovery makes use of schema.org and other metadata standards that can be added to the web pages (i.e. to the html files) that describe datasets. Google's Dataset Search tool "understands" metadata structured using either schema.org Dataset markup, or equivalent structures represented in W3C's Data Catalog Vocabulary ([DCAT](https://www.w3.org/TR/vocab-dcat/#dcat-scope)) format. 

How does that relate to the standards and schemas recommended in this GUide? Schemas from schema.org and DCAT are thus intended to make data more <u>discoverable</u>; they are not primarily intended to make the data more <u>usable</u>. We therefore recommend to document data using the most appropriate standard or schema for each type of data (DDI, Dublin Core, ISO 19139, etc.; see section 1.4), and to map (part of) the content of the metadata to the schema.org.  
