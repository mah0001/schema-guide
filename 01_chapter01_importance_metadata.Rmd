...
Search engines like Google and Bing are designed to search the whole world wide web, not specifically to find data. They perform well for data queries when a simple answer can be provided (e.g., "population of India in 2020"), or when a user searches for a specific resource available on-line (e.g., "Living Standards Measurement Survey Albania 2012"). But they will not and should not be expected to perform as well for more complex or specialized data queries. Researchers will thus often turn to specialized on-line data catalogs, like the open data platforms and catalogs maintained by national or international organizations and academic data centers and libraries. Unfortunately, the search engine algorithms embedded in such catalogs are often out-of-the-box solutions that also perform poorly, in part due to the low quality of the metadata attached to the datasets, and in part due to the lack of optimization of the search algorithm. Google's *Dataset Search* is an attempt to apply the performance of Google's search engine to a specialized data catalog. But its performance is also hampered by the paucity of available metadata. The examples below (generated in January 2022; results may differ over time) illustrate in an anecdotal manner some of the problems that researchers face when searching for data.

**Example 1: Searching for *"microdata on inequality in Africa"* in [Google](https://www.google.com/search?q=microdata+on+inequality+in+Africa&rlz=1C1GCEA_enUS977US977&oq=microdata+on+inequality+in+Africa&aqs=chrome.0.69i59j69i64.1478j0j4&sourceid=chrome&ie=UTF-8)**

As a response to the query *microdata on inequality in Africa*, Google returns as the most relevant results a list of <u>publications</u> (working papers and citations), not links to micro-datasets. Although the returned publications may eventually lead the data analysts to some useful datasets, the results fail to provide the user with what was specifically requested:  <u>microdata</u>.

<br>
<center>
![](./images/search_issue_google.JPG){width=90%}
</center>
<br>

**Example 2: Searching for *"US economic growth"* in the [US Open Data platform (data.gov)](https://catalog.data.gov/dataset?q=us+economic+growth&sort=score+desc%2C+name+asc&as_sfid=AAAAAAVQcFJrQSuZ7Phfhyds5Cu7FSjgZkk6_BNOAo80l8Jlt7fP9PYExdfT9Qpv8_nnZtG1WdBai1gfGn4cpPUiL3Otc1NU44xrjydWs1_F9P2F1nwhk_mqUccQeVOOeefhT_g%3D&as_fid=0ea219634a9a055f6599275e489e6f0e4b97e917)**

The results to a query for *US economic growth* submitted to data.gov illustrates a different problem: the relevancy and ranking of results. The platform returns as "most relevant" a series of datasets related to the growth of sablefish. Data on GDP growth or other economic indicators (available in data.gov) would have been more appropriate. When searching data catalogs, most users will expect an experience similar to the one they get from the lead search engines, and may fail to explore the list of results beyond the top entries. 

<br>
<center>
![](./images/search_issue_data_gov.JPG){width=90%}
</center>
<br>

**Example 3: Searching for *"dutch disease"* in the [World Bank's Development Data Hub](https://datacatalog.worldbank.org/int/search?q=dutch%20disease&start=0&sort=)**
...
<br>
<center>
![](./images/search_issue_wb_ddh_dutch_disease.JPG){width=90%}
</center>
<br>

**Example 4: Searching for *"gdp per capita india 2020"* in [Google](https://www.google.com/search?q=gdp+per+capita+india+2021&rlz=1C1GCEA_enUS977US977&ei=ehrjYffSBdCKggfE5K-QAg&start=0&sa=N&ved=2ahUKEwj394WWubT1AhVQheAKHUTyCyI4MhDy0wN6BAgBEDc&biw=1600&bih=650&dpr=1.2)**

Search engine like Google are increasingly designed to return not just links, but **answers**. This is intended to provide improved user experience, and to service digital assistants (like Hey Google, Alexa, Siri, and others). Advanced machine learning and natural language processing (NLP) solutions are implemented to generate and rank results, and to provide the "best answer". By doing this, these search engines operate choices which are not always to the advantage of specialized audience and data producers. When searching for *gdp per capita india 2021* for example, Google provides an immediate answer by extracting information (as of 15 January 2022) from a commercial source, not from the Ministry of Statistics (MOSPI) which is the official producer of GDP estimates for India. 

<br>
<center>
![](./images/gdp_india_1.JPG){width=90%}
</center>
<br>
<br>
<center>
![](./images/gdp_india_2.JPG){width=90%}
</center>
<br>
<br>
MOSPI's website only appears in the 6th page of results, which few users will ever access. For many users, the source of the information may not matter; but for others, a link to the originating and "reference" source - where more metadata can possibly be found - may be critical.
<br>
<br>
<center>
![](./images/gdp_india_3.JPG){width=90%}
</center>
<br>
