...
The ranking of a web page by Google or other search engine is determined by a complex and proprietary algorithm, and is controlled by Google. Unless ad, cannot guarantee that will be on top. For ranking, content is what matters most; rich metadata will include better description, so better indexed information about each dataset. Invest in data documentation and pay attention to keywords, topics, description, etc. specific to the dataset. Also, links from other web pages. Web developers can also take action to improve it by following the Search Engine Optimization (SEO) recommendations and guidelines provided by search engines. One of these recommendations is to embed "structured data" in the catalog's HTML pages. 

What do we mean by "structured data" in this context?
